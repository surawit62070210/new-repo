...
router.put('/update/amount', async function (req, res, next) {
  ...
  try {
    await pool.query(
      'UPDATE profile SET amount=? WHERE id=?', [amount, id]
    )
    await pool.query(
      'UPDATE profile SET code=? WHERE id=?', [code, id]
    )
    await conn.commit()
    res.send("success!");
  } catch (error) {
...
router.put('/update/status', async function (req, res, next) {
  ...
  try {
    ...
    //statusProfile
    await pool.query(
      'UPDATE profile SET status=? WHERE id=?', [status, id]
    )
    if (status == "Pay") {
      //statusProfile
      await pool.query(
        'UPDATE profile SET amount=? WHERE id=?', [0, id]
      )
      await pool.query(
        'UPDATE profile SET code=? WHERE id=?', ["", id]
      )

      //statusStadium1
      await pool.query(
        'UPDATE stadium1 SET status=? WHERE username=?', ["empty", username]
      )
      await pool.query(
        'UPDATE stadium1 SET username=? WHERE username=?', ["", username]
      )

      //statusStadium2
      await pool.query(
        'UPDATE stadium2 SET status=? WHERE username=?', ["empty", username]
      )
      await pool.query(
        'UPDATE stadium2 SET username=? WHERE username=?', ["", username]
      )
...
router.put('/user/changepass', async (req, res, next) => {
  ...
  try {
    ...
    await pool.query(
      'UPDATE profile SET password=? WHERE username=?', [newpass, username]
    )
